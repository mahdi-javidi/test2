...
-- =====================================================================
-- Consolidate into users and posts_new, then drop old user/users and post/posts
-- =====================================================================
USE `arcade`;
...
CREATE TABLE IF NOT EXISTS posts_new (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NULL DEFAULT NULL,
  CONSTRAINT fk_posts_new_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- Migrate users -> users (preserve IDs)
INSERT INTO users (id, username, password_hash, email, profile_picture, status, phone, created_at, updated_at)
SELECT u.id, u.username, u.password_hash, NULL, NULL, 'active', NULL, u.created_at, NULL
FROM users u
WHERE NOT EXISTS (SELECT 1 FROM users sn WHERE sn.id = u.id);

-- Migrate admin user -> users (assign new IDs, avoid username duplicates)
INSERT INTO users (username, password_hash, email, profile_picture, status, phone, created_at, updated_at)
SELECT a.username, a.password, a.email, a.profile_picture, a.status, NULL, a.created_at, NULL
FROM `user` a
WHERE NOT EXISTS (SELECT 1 FROM users sn WHERE sn.username = a.username);
...
ALTER TABLE comments
  ...
  ADD CONSTRAINT fk_comments_user_new FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;

ALTER TABLE likes
  ADD CONSTRAINT fk_likes_post_new FOREIGN KEY (post_id) REFERENCES posts_new(id) ON DELETE CASCADE,
  ADD CONSTRAINT fk_likes_user_new FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
...
'INSERT INTO users (username, password_hash, email, profile_picture, status, created_at)
